...
const validateField = (
  ...
  message: string,
  res: Response,
  optionalValidationFunction?: (field: any) => boolean,
) => {
...
const validateMiddlewares = {
  ...
  validateClienteRegisterBody: async (
    ...
    const { nome, email, senha, cpf, cep, endereco, telefone, dataNasc } =
      req.body
    ...
    if (endereco) {
      ...
      res.status(400).json({ messsage: "Informe o endereço corretamente." })
    }
  ...
  validateUzerRegisterBody: async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    validateField(
      categoriaServico,
      "A categoria do serviço do uzer é inválida",
      res,
    )
  ...
  validateCreatePedidoBody: async (
    req: Request,
    res: Response,
    next: NextFunction,
  ) => {
    ...
    const { tipoPedido, titulo, nomeServico, descricao, userId } = req.body
